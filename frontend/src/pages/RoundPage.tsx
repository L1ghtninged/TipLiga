...
import { getRounds, getRoundMatches } from "../api/rounds";
import type { Match } from "../types/Match";

import { getTips, saveTip } from "../api/tips";
...
function RoundPage() {
  ...
  const hasTippablePostponedMatch =
    selectedRound?.is_closed &&
    matches.some((match) => match.stav === "postponed");
  ...
  function handleJokerChange(matchId: number) {
    // Po uzavření kola už nelze žolíka měnit.
  ...
  async function handleSave() {
    ...
    try {
      ...
      const newJokerEntry = Object.entries(tipInputs).find(
        ([, input]) => input.is_joker,
      );

      const newJokerId = newJokerEntry ? Number(newJokerEntry[0]) : null;

      // Žolíka lze měnit pouze v otevřeném kole.
      if (
        !selectedRound.is_closed &&
        oldJoker &&
        newJokerId !== null &&
  ...
  (selectedRound.is_closed && match.stav === "postponed");
        ...
        const jokerState = selectedRound.is_closed
          ? (existingTip?.is_joker ?? false)
          : input.is_joker;
        ...
        if (
          ...
          existingTip.is_joker === jokerState
        ) {
          continue;
        }
      ...
      console.error(error);
      setError(
        error instanceof Error ? error.message : "Nepodařilo se uložit tipy.",
      );
    } finally {
      setSaving(false);
    }
  ...
  function renderMatchDate(match: Match) {
    const formattedDate = formatMatchDate(match.zacatek_zapasu);

    if (match.stav === "postponed") {
      if (formattedDate) {
        return (
          <>
            🔄 <span>Nový termín: {formattedDate}</span>
          </>
        );
      }
      ...
      return (
        <>
          ⏸️ <span>Odloženo — nový termín není znám</span>
        </>
      );
    }
  ...
  (isClosed && match.stav === "postponed");
          ...
          return (
            <div
              ...
              } ${match.stav === "postponed" ? "postponed" : ""} ${
            ...
            >
              ...
              <div className="match-card-header">
                <span
                  className={`match-date ${
                    match.stav === "postponed" ? "match-date-postponed" : ""
                  }`}
                >
              ...
              <div className="match-card-body">
                ...
                <div className="team team-home">
                  <span className="team-name">{match.domaci_tym.nazev}</span>
                ...
                <div className="team team-away">
                  ...
                  <span className="team-name">{match.hostujici_tym.nazev}</span>
                </div>
              </div>
              ...
              {isClosed && match.stav === "played" ? (
                <div className="match-result">
                  <div className="actual-result-label">Skutečný výsledek</div>
                  ...
                  {input && (
                    <div
                      className={`points-earned ${
                        tips.find((tip) => tip.zapas_id === match.id)?.is_joker
                          ? "joker-points"
                          : ""
                      }`}
                    >
                      +
                      {tips.find((tip) => tip.zapas_id === match.id)
                        ?.body_ziskane ?? 0}{" "}
                      bodů
                    </div>
                  )}
      ...
      {/* Tlačítko uložení */}
      {(!selectedRound?.is_closed || hasTippablePostponedMatch) && (
        <button
