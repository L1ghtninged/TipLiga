# app/routes/admin_routes.py

import logging

from flask import Blueprint, request, jsonify
from app.services.admin_service import AdminService as service
from app.routes.exceptions import *
from app.services.vyhodnoceni_service import VyhodnoceniService as vyhodnoceni
from app.utils.security import admin_required
from app.utils.validation import get_json_data
from flask import current_app

admin_bp = Blueprint(
    "admin",
    __name__,
    url_prefix="/api/admin"
)


@admin_bp.route("/users", methods=["POST"])
@admin_required
def create_user():

    data = get_json_data()
    user = service.new_user(data)

    current_app.logger.info(
        "Admin created user %s (ID %s) from IP %s.",
        user.username,
        user.id,
        request.remote_addr
    )

    return jsonify({
        "message": "User created",
        "user": user.to_dict()
    }), 201


@admin_bp.route("/users/<username>", methods=["DELETE"])
@admin_required
def delete_user(username):

    user = service.delete_user(username)

    current_app.logger.info(
        "Admin deleted user %s (ID %s) from IP %s.",
        user.username,
        user.id,
        request.remote_addr
    )

    return jsonify({
        "message": "User deleted",
        "user": user.to_dict()
    }), 200


@admin_bp.route("/users", methods=["GET"])
@admin_required
def get_all_users():

    users = service.get_all_users()

    return jsonify([
        user.to_dict()
        for user in users
    ]), 200


@admin_bp.route("/users/<int:user_id>/tips", methods=["GET"])
@admin_required
def get_user_tips(user_id):

    tips = service.get_tips_for_user(user_id=user_id)

    return jsonify({
        "user_id": user_id,
        "tips": [tip.to_dict() for tip in tips]
    }), 200


@admin_bp.route("/rounds", methods=["POST"])
@admin_required
def create_rounds():

    data = get_json_data()
    count = data.get("pocet_kol", 1)

    if not isinstance(count, int) or count <= 0:
        raise ValidationError("Count must be a positive integer")

    if count > 50:
        raise ValidationError(
            "You can create at most 50 rounds at once."
        )

    service.create_rounds(count)

    current_app.logger.info(
        "Admin created %s rounds from IP %s.",
        count,
        request.remote_addr
    )

    return jsonify({
        "message": "Rounds created",
        "count": count,
    }), 201


@admin_bp.route("/rounds/<int:round_number>", methods=["DELETE"])
@admin_required
def delete_round(round_number):

    kolo = service.delete_round(round_number)

    current_app.logger.info(
        "Admin deleted round %s (ID %s) from IP %s.",
        round_number,
        kolo.id,
        request.remote_addr
    )

    return jsonify({
        "message": "Round deleted",
        "id": kolo.id
    }), 200


@admin_bp.route("/rounds", methods=["DELETE"])
@admin_required
def delete_rounds():

    data = get_json_data()
    ids = data.get("round_ids", [])

    for round_id in ids:
        try:
            service.delete_round(round_id)

            current_app.logger.info(
                "Admin deleted round %s from IP %s.",
                round_id,
                request.remote_addr
            )

        except Exception:
            current_app.logger.exception(
                "Error deleting round %s.",
                round_id
            )

            return jsonify({
                "error": "Internal server error."
            }), 500

    return jsonify({
        "message": "Rounds deleted"
    }), 200


@admin_bp.route("/rounds", methods=["GET"])
@admin_required
def get_all_rounds():

    rounds = service.get_all_rounds()

    return jsonify([
        kolo.to_dict()
        for kolo in rounds
    ]), 200


@admin_bp.route("/teams", methods=["GET"])
@admin_required
def get_all_teams():

    teams = service.get_all_teams()

    return jsonify([
        team.to_dict()
        for team in teams
    ]), 200


@admin_bp.route("/teams", methods=["POST"])
@admin_required
def create_team():

    data = get_json_data()
    team_name = data.get("nazev")

    team = service.create_team(team_name)

    current_app.logger.info(
        "Admin created team %s (ID %s) from IP %s.",
        team.nazev,
        team.id,
        request.remote_addr
    )

    return jsonify({
        "message": "Team created",
        "team": team.to_dict()
    }), 201


@admin_bp.route("/teams/<int:team_id>", methods=["DELETE"])
@admin_required
def delete_team(team_id):

    service.delete_team(team_id)

    current_app.logger.info(
        "Admin deleted team %s from IP %s.",
        team_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Team deleted",
        "id": team_id
    }), 200


@admin_bp.route("/teams/<int:team_id>/logo", methods=["PUT"])
@admin_required
def add_logo_url(team_id):

    data = get_json_data()
    logo_url = data.get("logo_url")

    service.add_logo_url(team_id, logo_url)

    current_app.logger.info(
        "Admin updated logo URL for team %s from IP %s.",
        team_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Logo URL added",
        "team_id": team_id,
        "logo_url": logo_url
    }), 200


@admin_bp.route("/matches", methods=["POST"])
@admin_required
def create_match():

    data = get_json_data()
    match = service.create_match(data)

    current_app.logger.info(
        "Admin created match %s (round %s) from IP %s.",
        match.id,
        match.kolo_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Match created",
        "match": match.to_dict()
    }), 201


@admin_bp.route("/matches/<int:match_id>", methods=["DELETE"])
@admin_required
def delete_match(match_id):

    service.delete_match(match_id)

    current_app.logger.info(
        "Admin deleted match %s from IP %s.",
        match_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Match deleted",
        "match_id": match_id
    }), 200


@admin_bp.route("/matches/<int:match_id>", methods=["PUT"])
@admin_required
def update_match(match_id):

    data = get_json_data()

    domaci_skore = data.get("domaci_skore")
    hostujici_skore = data.get("hostujici_skore")

    match = service.update_match_score(
        match_id,
        domaci_skore,
        hostujici_skore
    )

    current_app.logger.info(
        "Admin updated score of match %s from IP %s.",
        match_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Match updated",
        "match": match.to_dict()
    }), 200


@admin_bp.route("/rounds/<int:round_id>/matches", methods=["GET"])
@admin_required
def get_matches_by_round(round_id):

    matches = service.get_matches_by_round(round_id)

    return jsonify([
        match.to_dict()
        for match in matches
    ]), 200


@admin_bp.route("/matches/<int:match_id>/reschedule", methods=["PUT"])
@admin_required
def reschedule_match(match_id):

    data = get_json_data()

    novy_cas = data.get("zacatek_zapasu")
    stav = data.get("stav")

    match = service.update_stav_a_cas(
        match_id,
        novy_cas,
        stav
    )

    current_app.logger.info(
        "Admin rescheduled match %s to state %s from IP %s.",
        match_id,
        stav,
        request.remote_addr
    )

    return jsonify({
        "message": "Match rescheduled",
        "match": match.to_dict()
    }), 200


@admin_bp.route("/matches/<int:match_id>/close", methods=["PUT"])
@admin_required
def close_match(match_id):

    match = service.update_stav_a_cas(
        match_id,
        None,
        "played"
    )

    vyhodnoceni.calculate_match(match)

    current_app.logger.info(
        "Admin closed match %s and calculated points from IP %s.",
        match_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Match closed and points assigned",
        "match": match.to_dict()
    }), 200


@admin_bp.route("/tips/<int:match_id>", methods=["GET"])
@admin_required
def get_tips_for_match(match_id):

    tips = service.get_all_users_with_tips_for_match(match_id)

    return jsonify([
        tip.to_dict()
        for tip in tips
    ]), 200


@admin_bp.route("/rounds/<int:round_id>/close", methods=["PUT"])
@admin_required
def close_round(round_id):

    kolo = service.close_round(round_id)

    current_app.logger.info(
        "Admin closed round %s from IP %s.",
        round_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Round closed",
        "round": kolo.to_dict()
    }), 200


@admin_bp.route("/rounds/<int:round_id>/reopen", methods=["POST"])
@admin_required
def reopen_round(round_id):

    try:
        service.reopen_round(round_id)

        current_app.logger.info(
            "Admin reopened round %s from IP %s.",
            round_id,
            request.remote_addr
        )

        return jsonify({
            "message": "Kolo bylo znovu otevřeno."
        }), 200

    except ValueError as error:

        return jsonify({
            "error": str(error)
        }), 400


@admin_bp.route("/rounds/<int:round_id>/calculate", methods=["PUT"])
@admin_required
def calculate_round(round_id):

    matches = vyhodnoceni.calculate_round(
        round_id=round_id
    )

    current_app.logger.info(
        "Admin calculated round %s from IP %s.",
        round_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Tips calculated and round closed",
        "round_id": round_id,
        "matches": [
            match.to_dict()
            for match in matches
        ],
    }), 200


@admin_bp.route("/tips/<int:tip_id>", methods=["PUT"])
@admin_required
def change_tip(tip_id):

    data = get_json_data()

    domaci_predpoved = data.get(
        "predpoved_domaci_skore"
    )
    host_predpoved = data.get(
        "predpoved_hostujici_skore"
    )
    match_id = data.get("zapas_id")
    is_joker = data.get(
        "is_joker",
        False
    )

    tip = service.change_tip(
        tip_id,
        domaci_predpoved,
        host_predpoved,
        match_id,
        is_joker
    )

    vyhodnoceni.recalculate()

    current_app.logger.info(
        "Admin changed tip %s and recalculated points from IP %s.",
        tip_id,
        request.remote_addr
    )

    return jsonify({
        "message": "Tip changed. Points have been recalculated",
        "tip": tip.to_dict(),
    }), 200


@admin_bp.route("/recalculate", methods=["PUT"])
@admin_required
def recalculate():

    vyhodnoceni.recalculate()

    current_app.logger.info(
        "Admin recalculated all points from IP %s.",
        request.remote_addr
    )

    return jsonify({
        "message": "Points have been recalculated"
    }), 200


@admin_bp.route("/season", methods=["POST"])
@admin_required
def calculate_season():

    data = get_json_data()

    vyhodnoceni.evaluate_standing_points(
        standings_data=data
    )

    current_app.logger.info(
        "Admin evaluated season standings from IP %s.",
        request.remote_addr
    )

    return jsonify({
        "message": "Points have been adjusted"
    }), 200